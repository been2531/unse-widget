...
- 루틴 에이전트 설정 완료 (매 3시간, 진단→TASKS 추가→구현→PR, 세션당 95% 컨텍스트 활용)
...
### 교체 필수 (한국신화 컨셉 불일치)
- [ ] `lightning_1` 천붕이 — 노란 후드티 인간 꼬마 → 뇌신 영아 형태로 **[최우선]**
- [ ] `nature_3` 산신 — 중국 장군 스타일 → 조선 도포 입은 산신령으로
- [ ] `nature_4` 단군 — 중국 장군 → 고조선 제의복 신인으로
- [ ] `light_4` 환인 — 중국 신선 → 한국 최고천신으로
- [ ] `fire_4` 태양신조 — 실루엣만 보임 → 오색 깃털 디테일 살리기

### 스타일 보완 (치비 → 신화적으로)
- [ ] `fire_1` 봉아 — 일반 병아리 → 한국 오색 봉황 새끼로
- [ ] `water_1` 이무기 — kawaii 아기 뱀 → 용꿈 꾸는 신비로운 이무기로
- [ ] `dark_1` 도깨비 — 서양 고블린 느낌 → 뿔+도깨비방망이 한국 도깨비로

### 신규 생성 필요 (카드 데이터는 있으나 이미지 없음)
- [ ] `gumiho_1/2/3` — 미호·여우·구미호 시리즈 (fire 원소)
- [ ] `imugi_1/2/3` — 이미·수리·이무기 시리즈 (water 원소)
- [ ] `samjogo_1/2` — 삼미·해조 시리즈 (lightning 원소)
...
- [ ] 컬렉션 카드 상세에서 한국신화 설명(description) 표시 강화
- [ ] 빈 컬렉션 상태(empty state) 화면 — 카드 0장일 때 안내 UI
- [ ] 로딩 스켈레톤 — 데이터 로딩 중 빈 화면 대신 placeholder
- [ ] 스트릭 배지 1일차부터 표시 — `currentStreak >= 2` → `>= 1`로 완화
...
**[JS-only] 먼저 할 수 있는 것:**
- [ ] `src/storage/purchases.ts` — `remove_ads` 상태 저장/조회
- [ ] `coin-shop.tsx` 상단에 광고 제거 상품 카드 추가
- [ ] 운세 잠금 해제: 광고 제거 구매 시 광고 대신 50코인 차감으로 분기
