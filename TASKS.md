...
- ~~컬렉션 탭 여백 3곳 균일화~~ → 수집률바↔카테고리탭↔원소필터↔그리드 각 8px 균등
- ~~컬렉션 전체 스크롤 개선~~ → ListHeaderComponent로 진행바+탭+필터를 FlatList 안으로 이동
- ~~컬렉션 운세 카드 버프 정보 표시~~ → 모달에 버프 효과·적용 대상·활성화 조건 안내
- ~~홈 이번 주 흐름 개선~~ → 주간 요약(좋음N·보통N·나쁨N + 주 경향 레이블) 추가
- ~~홈 useFocusEffect~~ → 코인샵 구매/장착 후 홈 탭으로 돌아올 때 즉시 반영
- ~~가챠 컬렉션 아이콘 제거~~ → 탭바로 이동 가능하므로 헤더 아이콘 제거
...
- ~~`jujak_1/2/3`~~ → 주이·주이 희귀·주작 시리즈 완료 (jujak_1 주홍색으로 봉아와 차별화)
- ~~`bonghwang_1/2/3`~~ → 봉이·황이·봉황 시리즈 완료 (오색 평화 신조)
- ~~`dokkaebi_1/2/3`~~ → 도끼·귀신이·도깨비 시리즈 완료 (암흑 트릭스터)
- ~~`haetae_1/2/3`~~ → 해이·태태·해태 시리즈 완료 (정의 수호 사자)
- ~~`cheongnyong_1/2/3`~~ → 청이·녕이·청룡 시리즈 완료 (동방 수호 청룡)
- ~~`baekho_1/2/3`~~ → 백이·호이·백호 시리즈 완료 (서방 수호 백호)
- ~~`hyeonmu_1/2/3`~~ → 현이·무이·현무 시리즈 완료 (북방 수호 현무)

### 유지 (교체 불필요)
`fire_2/3`, `water_2/3/4`, `lightning_2/3/4`, `nature_1/2`, `dark_2/3/4`, `light_1/2/3`

### 카드 아트 배경 처리 방향 변경
- ~~**배경 유지로 전환** — 흰 배경 제거(BFS flood-fill) 대신 배경 살리기~~ → 완료
  ...
  - ~~전체 카드(38장) 배경 유지 버전으로 재생성 완료~~ (봉황/도깨비/해태/청룡/백호/현무 6개 시리즈 추가 포함)
